...
class RAGChain:
    ...
    def __init__(
        ...
        # Инициализируем клиент - OpenAI автоматически использует переменную окружения OPENAI_API_KEY
        # Если передаем явно, используем только api_key без других параметров
        # Это предотвращает конфликты с параметрами proxies или http_client
...
3. Отвечай на русском языке, четко и структурированно
4. При необходимости ссылайся на конкретные документы или разделы
